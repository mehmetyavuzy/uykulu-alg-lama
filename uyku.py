...
while ret:
    ...
    if len(faces) > 0:
        for (x, y, w, h) in faces:
            ...
            if len(eyes) >= 2:
                if first_read:
                    cv2.putText(image, "Goz algilandi, yanip sonmeyi kontrol etmek için s'ye basin", (70, 70), cv2.FONT_HERSHEY_SIMPLEX,
                                1, (0, 255, 0), 2)
                else:
                    cv2.putText(image, "Gozler acik", (70, 70), cv2.FONT_HERSHEY_SIMPLEX,
                                1, (255, 255, 255), 2)
            else:
                if first_read:
                    cv2.putText(image, "Goz algilanmadi", (70, 70), cv2.FONT_HERSHEY_SIMPLEX,
                                1, (255, 255, 255), 2)
                else:
                    cv2.putText(image, "Goz Kirpma Algilandi.....!!!!", (70, 70), cv2.FONT_HERSHEY_SIMPLEX,
                                1, (0, 255, 0), 2)
                    cv2.imshow('image',image)
                    cv2.waitKey(1)
                    print("Goz Kirpma Algilandi.....!!!!")
    else:
        cv2.putText(image, "Yuz Algılanmadı.", (70, 70), cv2.FONT_HERSHEY_SIMPLEX,
                    1, (0, 255, 0), 2)
    cv2.imshow('image', image)
    a = cv2.waitKey(1)
